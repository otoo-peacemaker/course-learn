...
class LoginFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        setAppButton(binding.loginBtn, "Log in") { login()
                ...
                binding.loader, onError = {
                    ...
                    if (it == unVerifyEmail) {
                        showRetrySnackBar(requireView(), it, "Verify") {
                            ...
                            val action =
                                LoginFragmentDirections.actionLoginFragmentToVerifyEmailFragment(
                                    email = email
                                )
                            navigateTo(action)
                        }
        ...
        binding.forgotPassword.setOnClickListener {
            ...
            printLogs("Email", email)
            val action =
                LoginFragmentDirections.actionLoginFragmentToResetPasswordFragment(email = email)
    ...
    private fun login() {
        with(binding) {
            ...
            if (validateString(email) and validateString(password)) viewModel.signIn(
                email,
                password
            )
            else showSnackBar(
                requireView(),
                "Field(s) can not be empty or must be greater than 3 characters"
            )
        }
    }

    private fun insertDummyData() {
        val course: List<CoursesItem>? =
            parseJsonFileToListOfDataClass(requireContext(), "courses.json")
        if (course != null) {
            // Use 'course' object
